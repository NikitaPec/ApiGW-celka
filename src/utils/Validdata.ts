...
class Validdata {
  ...
  checkLogin(login: string, response: ApiResponse) {
    if (login == "") {
  ...
  checkPassword(password: string, confirm: string, apiResponse: ApiResponse) {
    ...
    if (!validPass) apiResponse.setError("password", "Строчные и прописные буквы, цифры. От 6 до 12 символов");
  }
  ...
  async candidate(login: string, apiResponse?: ApiResponse, method?: string): Promise<User | null> {
    ...
    if (apiResponse) {
      ...
      if (user && method == "check")
        apiResponse.setError("login", `Пользователь с ${errorMessageType} ${loginDefinition} ${errorMessageMethod}.`);
